...
def get_product_links(driver, search_query=None, category_url=None, max_pages=DEFAULT_MAX_PAGES):
    links = set()
    base_url_wb = "https://www.wildberries.ru/" # Базовый URL для join
    for page in range(1, max_pages + 1):
        if category_url:
            # Используем urljoin и для URL категории для единообразия и безопасности
            page_url_segment = f"?page={page}"
            current_search_url = urljoin(category_url, page_url_segment)
        else:
            current_search_url = f"https://www.wildberries.ru/catalog/0/search.aspx?page={page}&search={search_query}"
        ...
        for a in soup.find_all("a", href=True):
            if "/catalog/" in a["href"] and "/detail.aspx" in a["href"]:
                ...
                # Используем urljoin для корректного формирования URL товара
...
def parse_product_page(driver, url):
    ...
    # Фильтрация по заданному диапазону цен
    PRICE_MIN_FILTER = 0.0
    PRICE_MAX_FILTER = 500.0

    # Применяем фильтр. Если final_current_price = 0.0, и 0.0 входит в диапазон, товар пройдет.
    if not (PRICE_MIN_FILTER <= final_current_price <= PRICE_MAX_FILTER):
        log_info(f"Товар '{product_name or '[Без названия]'}' ({url}) не проходит по цене: {final_current_price:.2f} руб. (диапазон: {PRICE_MIN_FILTER:.2f} - {PRICE_MAX_FILTER:.2f} руб.).")
        return {
            "status": "price_out_of_range", 
            "url": url, 
            "price": final_current_price, 
...
def get_driver(proxy=None, user_agent=None):
    ...
    # options.add_argument("--headless") # Временно отключаем headless
    options.add_argument("--disable-gpu")
    options.add_argument("user-agent=" + (user_agent if user_agent else get_random_user_agent()))
    
    # Временно включаем загрузку изображений (комментируем отключение)
